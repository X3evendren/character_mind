...
import type { AffectiveVector } from "../mind/emotion";
import { PsychologyResult, EmotionResult, AppraisalResult, MotivationResult, AttachmentResult, DefenseResult, RelationResult, PsychologyEngine } from "../mind/psychology";
...
export class FourLayerColdAnalyzer {
  ...
  private async analyzeLayer0(params: ColdAnalyzeParams): Promise<{ text: string; vector: AffectiveVector }> {
...
<tension>0.35</tension>`;
    ...
    return { text, vector: { warmth: parse("warmth"), weight: parse("weight"), clarity: parse("clarity"), tension: parse("tension") } };
  }

  private async analyzeLayer1(params: ColdAnalyzeParams, layer0: { text: string; vector: AffectiveVector }): Promise<string> {
    ...
    const prompt = `感知上一刻的情绪回声。

上次情绪: ${prevEmo.emotionDominant} (${(prevEmo.emotionIntensity * 100).toFixed(0)}%)
过去了: ${secondsSince.toFixed(0)}秒
底色: ${layer0.text}
用户说: ${params.input.slice(0, 200)}

请只输出1句话中文。如果上一刻的感受还在回荡，描述那种余韵。如果已经完全消散了，输出: 无
格式: 只输出这句话本身,不要标签,不要解释`;

    const resp = await this.psychProvider.chat(
      [{ role: "user", content: prompt }], 0.3, 1000, undefined, "",
    );
  ...
  private async analyzeLayer2(params: ColdAnalyzeParams, layer0Text: string, layer1Text: string): Promise<PsychologyResult> {
    ...
    return engine.analyze(
      { description: params.input, type: params.taskMode ? "tool_use" : "social", significance: 0.5, layer0Context: layer0Text, layer1Context: layer1Text },
      params.snapshot, params.mindState, params.drives, params.assistantConfig, affectiveContext,
    );
  }

  private async analyzeLayer3(params: ColdAnalyzeParams, layer0Text: string, layer1Text: string, psych: PsychologyResult): Promise<string> {
