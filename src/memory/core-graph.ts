...
export class CoreGraphMemory extends MemoryStore {
  ...
  querySubgraph(entity: string, depth = 2): any {
    ...
    for (let d = 0; d < depth && frontier.length; d++) {
      ...
      for (const er of edgeRows) {
        relevantEdges.push(er);
        if (!visited.has(er.from_id)) { visited.add(er.from_id); nf.push(er.from_id); }
        if (!visited.has(er.to_id)) { visited.add(er.to_id); nf.push(er.to_id); }
      }
      frontier = nf;
    }
    const now = Date.now() / 1000;
    const decayedEdges = relevantEdges.map(er => ({
      from: er.from_id, to: er.to_id, relation: er.relation,
      weight: Math.max(0.1, er.weight * Math.exp(-(now - er.timestamp) / (this.halfLifeDays * 86400))),
      source: er.source_event,
    }));
    const nodeData = [...visited].map(nid => {
      const nr = this._db!.get("SELECT node_id, label, node_type FROM nodes WHERE node_id=?", nid) as any;
      return nr ? { id: nr.node_id, label: nr.label, type: nr.node_type } : null;
    }).filter(Boolean) as any[];
    const summary = decayedEdges.slice(0, 10).map(e =>
      `${nodeData.find(n => n.id === e.from)?.label ?? e.from}与${nodeData.find(n => n.id === e.to)?.label ?? e.to}: ${e.relation}`
    ).join("; ") || "无显著关系";
  ...
  private _addEdge(fid: string, tid: string, rel: string, now: number, src: string): void {
    const ex = this._db!.get(
      "SELECT edge_id, weight FROM edges WHERE from_id=? AND to_id=? AND relation=? AND superseded=0",
      fid, tid, rel,
    ) as any;
    if (ex)
      this._db!.run("UPDATE edges SET weight=MIN(1.0,?), timestamp=? WHERE edge_id=?", ex.weight + 0.2, now, ex.edge_id);
    else
      this._db!.run("INSERT INTO edges (from_id,to_id,relation,weight,timestamp,source_event) VALUES (?,?,?,?,?,?)", fid, tid, rel, 0.5, now, src);
  }
