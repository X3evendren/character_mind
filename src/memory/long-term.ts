/** Long-Term Memory — SQLite (sql.js) + time decay. */
...
export class LongTermMemory extends MemoryStore {
  ...
  get length(): number {
    return this._db ? (this._db.get("SELECT COUNT(*) as c FROM ltm WHERE superseded=0") as any).c : 0;
  }
  ...
  async store(record: MemoryRecord): Promise<string> {
    ...
    record.recordId = rid;
    this._db!.run("INSERT OR REPLACE INTO ltm VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
  ...
  async recall(query: string, n = 5): Promise<MemoryRecord[]> {
    ...
    for (const [_, r] of scored.slice(0, n)) this._db!.run("UPDATE ltm SET recall_count=recall_count+1 WHERE record_id=?", r.recordId);
    this._db!.save();
  ...
  async consolidate(): Promise<ConsolidationReport> {
    ...
    const rows = this._db!.all("SELECT record_id, content FROM ltm WHERE superseded=0 ORDER BY timestamp DESC LIMIT 50") as any[];
    const seen = new Set<string>();
    for (const row of rows) {
      const rid: string = row.record_id;
      const content: string = row.content;
      if (seen.has(content)) {
        this._db!.run("UPDATE ltm SET superseded=1 WHERE record_id=?", rid);
        report.merged++;
      }
      seen.add(content);
    }
  ...
  detectContradictions(): Array<Record<string, string>> {
    const rows = this._db!.all("SELECT * FROM ltm WHERE superseded=0 ORDER BY timestamp DESC LIMIT 100") as any[];
    const conflicts: Array<Record<string, string>> = [];
    for (let i = 0; i < rows.length; i++) {
      for (let j = i + 1; j < rows.length; j++) {
        const e1 = JSON.parse(rows[i].emotion || "{}"), e2 = JSON.parse(rows[j].emotion || "{}");
  ...
  promoteCandidates(): MemoryRecord[] {
    return (this._db!.all("SELECT * FROM ltm WHERE recall_count>=5 AND significance>=0.8 AND superseded=0") as any[]).map((r: any) => this._rowToRecord(r));
  }

  private _timeDecay(r: MemoryRecord, now: number): number { return Math.exp(-(now - r.timestamp) / (this.halfLifeDays * 86400)); }
  private _trim(): void {
    const c = (this._db!.get("SELECT COUNT(*) as c FROM ltm") as any).c;
    if (c > this.maxItems) this._db!.run(
      "DELETE FROM ltm WHERE rowid IN (SELECT rowid FROM ltm ORDER BY significance ASC, timestamp ASC LIMIT ?)",
      c - this.maxItems,
    );
  }
  ...
  decayConfidence(halfLifeSeconds: number, now: number): number {
    ...
    const result = this._db!.run(
      `UPDATE ltm SET confidence = confidence * ?
       ...
       AND (? - timestamp) > ?`,
      Math.exp(-lambda), now, halfLifeSeconds,
    );
    this._db!.save();
    return result.changes;
  }
  ...
  extractPatterns(minClusterSize = 3): number {
    ...
    for (const [key, cluster] of clusters) {
      ...
      for (const r of cluster) {
        this._db!.run("UPDATE ltm SET significance = significance * 0.6 WHERE record_id=?", r.recordId);
        this._db!.run("UPDATE ltm SET recall_count = recall_count + 1 WHERE record_id=?", r.recordId);
      }
    }
  ...
  private _rowToRecord(row: any): MemoryRecord {
    return createMemoryRecord({
      ...
      emotionalSignature: JSON.parse(row.emotion || "{}"),
      significance: row.significance, eventType: row.event_type,
      tags: JSON.parse(row.tags || "[]"), timestamp: row.timestamp,
      recallCount: row.recall_count,
      memoryType: (row.memory_type as any) ?? "episodic",
      confidence: row.confidence ?? 0.7,
      superseded: !!row.superseded,
      supersededBy: row.superseded_by ?? null,
      metadata: { relatedIds: JSON.parse(row.related_ids || "[]"), embedding: row.embedding },
    });
  }
}
